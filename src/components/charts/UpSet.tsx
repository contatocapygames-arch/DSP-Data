 ...
 */
export function UpSet({ sets, combos, tooltip }: Props) {
  ...
  return (
    <div className="chart-scroll">
      <svg className="upset" width={width} height={height} role="img" aria-label="Combinações de anunciantes por usuários únicos">
        {ticks.map((t) => {
        ...
        {sets.map((s, i) => (
          <g key={s}>
            {i % 2 === 0 && (
              <rect x={LABEL_W - 4} y={gridTop + i * ROW} width={combos.length * COL + 8} height={ROW} className="zebra" />
            )}
