...
export function Heatmap({ labels, colLabels, value, intensity, format, tooltip, diagonal, exportAs }: Props) {
  ...
  const exporter: ChartExport = {
    ...
    table: () => ({
      header: ["", ...cols],
      rows: labels.map((l, i) => [l, ...cols.map((_, j) => value(i, j))]),
    }),
  };
  ...
  return (
    <>
      ...
      <div className="chart-scroll">
        <svg ref={svgRef} className="heatmap" width={width} height={height} role="img" aria-label={exportAs}>
          {cols.map((l, j) => (
            <text
              key={`c${j}`}
